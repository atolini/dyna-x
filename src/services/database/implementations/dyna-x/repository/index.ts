...
export class DynaXRepository<T> implements IRepository<T, Key, ConditionExpressionResult, UpdateExpressionResult> {
     ...
     */
    async batchWriteItems(putItems: T[], deleteKeys: Key[] = []): Promise<Array<{ type: 'put' | 'delete', item: T | Key }>> {
        ...
        await Promise.all(batchs.map(async (batch, i, batchs) => {
            ...
            const response: BatchWriteItemCommandOutput = await this.client.send(command);
            ...
            if (response.UnprocessedItems?.[tableName]) {
                const humanReadableItems = response.UnprocessedItems[tableName].map(unprocessed => {
                    if (unprocessed.PutRequest) {
                        return {
                            type: 'put' as const,
                            item: unmarshall(unprocessed.PutRequest.Item!) as unknown as T
                        };
                    } else {
                        return {
                            type: 'delete' as const,
                            item: unmarshall(unprocessed.DeleteRequest!.Key!) as unknown as Key
                        };
                    }
                });
     ...
     */
    async query(condition: ConditionBuilder, indexName?: string): Promise<T[] | null> {
        ...
        return response.Items ? response.Items.map((i) => unmarshall(i)) as T[] : null;
    }
     ...
     */
    async update(update: UpdateBuilder, key: Key, condition?: ConditionBuilder): Promise<T | null> {
        const {
            UpdateExpression,

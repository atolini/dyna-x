...
describe('CloudWatchLogService', () => {
  ...
  describe('putLog', () => {
    it('should send logs successfully with sequence token', async () => {
      ...
      const sentPutLogCommand =
        cloudwatchMock.commandCalls(PutLogEventsCommand)[0].args[0].input;
      ...
      expect(sentPutLogCommand.logEvents?.[0].message).toBe(
    ...
    it('should throw error when DescribeLogStreams fails', async () => {
      ...
      cloudwatchMock
        ...
        .rejects(new Error('Describe failed'));

      await expect(logService.putLog(logs, container)).rejects.toThrow(
        'Describe failed',
      );
    });
